      ...
      order => {
        ...
        if (order && this.count == 2 && this.last !== order._id) {
          ...
          order.products.forEach(
            item =>
              (itemsTXT += `${item.prod_name}\tx ${item.quantity} units\ttotal:  ${item.prod_total.toFixed(2)} Rupees\n\n`)
          );
          let doc = new jsPDF();
          doc.setFontSize(22);
          doc.setFontStyle("bold");
          doc.text("Thank you for shopping at Our Website", 20, 20);
          ...
          doc.setFontStyle("bold");
          doc.text("Total price: " + order.total.toFixed(2) + " Rupees", 10, 90);
